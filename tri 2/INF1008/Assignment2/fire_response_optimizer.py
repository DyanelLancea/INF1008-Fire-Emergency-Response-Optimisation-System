...
class FireResponseOptimizer:
    ...
    def find_optimal_route_with_trace(
        self,
        emergency_location: Tuple[float, float],
        fire_station_id: Optional[str] = None,
    ) -> Tuple[List[str], float, List[str]]:
        """
        Find optimal route and also return Dijkstra's visit order for visualization.
        """
        if self.dijkstra is None:
            self.initialize()
        ...
        if fire_station_id:
            ...
            station_node, _, _ = self.dijkstra.find_nearest_fire_station(
                emergency_node, list(self.fire_stations.keys())
            )

        path, distance, visit_order = self.dijkstra.find_shortest_path_with_trace(
            station_node, emergency_node
        )
