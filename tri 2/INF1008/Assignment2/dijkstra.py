...
class Dijkstra:
    ...
    def shortest_path_with_trace(
        self, start: str, end: Optional[str] = None
    ) -> Tuple[Dict, Dict, List[str]]:
        """
        Find shortest path(s) and also return the order in which nodes were visited.

        This is useful for visualizing how Dijkstra's algorithm explores the graph.
        """
        distances = {node: float("inf") for node in self.graph}
        distances[start] = 0
    ...
    def find_shortest_path_with_trace(
        ...
        """
        Find the shortest path from start to end and also return Dijkstra's visit order.
        """
        distances, previous, visit_order = self.shortest_path_with_trace(start, end)
        path = self.reconstruct_path(previous, start, end)
        distance = distances.get(end, float("inf"))

        return path, distance, visit_order
